...
df = spark.read \
  .format("bigquery") \
  .load("gothic-talent-435511-s2.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "data2_temp"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'gothic-talent-435511-s2.labdataset.wordcounts') \
  .mode("append") \
  .save()
